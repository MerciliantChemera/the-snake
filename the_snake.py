...
GRID_CENTER: tuple[int, int] = (GRID_WIDTH // 2, GRID_HEIGHT // 2)
...
class GameObject:
    ...
    def _draw_cell(
        ...
        # Переводим из координат поля в координаты окна
        rect = pg.Rect(
...
class Apple(GameObject):
    ...
    def randomize_position(
        ...
        self.position = excluded_positions[0]
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.KEYDOWN:
            ...
            if event.key in (pg.K_UP, pg.K_DOWN, pg.K_LEFT, pg.K_RIGHT):
                direction_info = UPDATE_ROTATION[event.key]
                if snake.direction != direction_info['conflict']:
                    snake.next_direction = direction_info['direction']
            # Пауза
